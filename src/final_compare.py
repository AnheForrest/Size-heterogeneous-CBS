"""
final_compare.py
HCBS-base vs 本文算法，困难场景（长条占比80%，N=5）
"""
...
OUT_DIR = os.path.join("..", "test", f"final_compare_{timestamp}")
...
print("\n" + "="*50)
print("对比表格：")
print(f"{'算法':<15} {'成功率':<10} {'平均求解时间(s)':<15}")
print("-"*50)
for alg in ['hcbs', 'ours']:
    print(f"{alg:<15} {summary[alg]['sr']:.1f}% ({summary[alg]['succ']}/{REPEAT})   {summary[alg]['avg_time']:.3f}")

table_file = os.path.join(OUT_DIR, "comparison_table.txt")
with open(table_file, 'w', encoding='utf-8') as f:
    f.write("HCBS-base vs 本文算法 对比结果\n")
    f.write("="*50 + "\n")
    f.write(f"场景：长条占比80%，N=5，重复{REPEAT}次\n")
    f.write(f"{'算法':<15} {'成功率':<10} {'平均求解时间(s)':<15}\n")
    f.write("-"*50 + "\n")
    for alg in ['hcbs', 'ours']:
        f.write(f"{alg:<15} {summary[alg]['sr']:.1f}% ({summary[alg]['succ']}/{REPEAT})   {summary[alg]['avg_time']:.3f}\n")
