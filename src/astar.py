"""
A* 低层规划模块
为单个智能体规划路径，考虑拥堵系数、桥栅格和全局时空预约表软约束。
"""
...
def astar(agent_instance: AgentInstance,
          ...
          weight_res: float = 1.0) -> Optional[List[Tuple[int, int]]]:
    """
    使用 A* 算法为单个智能体规划路径。
    ...
    # 优先队列元素：(f, g, t, pos)
    # 使用负 g 作为次级键以保证小 g 优先（可选）
    open_set = []
    # 初始状态
    t0 = 0
    g0 = step_penalty(start, t0)  # 起点时刻的惩罚计入代价
    ...
    while open_set:
        ...
        # 扩展等待动作
        nt = t + 1
        ng = g + 1 + step_penalty(pos, nt)  # 等待时位置不变
        state = (pos, nt)
        if ng < g_score.get(state, float('inf')):
            g_score[state] = ng
